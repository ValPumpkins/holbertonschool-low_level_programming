#ifndef MAIN_H
#define MAIN_H

/**
 * int _isupper(int c) : checks for uppercase character
 */

int _isupper(int c);

/**
 * int _isdigit(int c) : checks for a digit (0 through 9)
 */

int _isdigit(int c);

/**
 * int mul(int a, int b) : multiplies two integers
 */

int mul(int a, int b);

/**
 * void print_numbers(void) - prints the numbers from 0 to 9, follow by newline
 */

void print_numbers(void);

/**
 * _putchar - writes the character c to stdout
 */

int _putchar(char c);

/**
 * void print_most_numbers(void) - prints the numbers from 0 to 9 (without 2 & 4)
 */

void print_most_numbers(void);

/**
 * void more_numbers(void) - prints 10 times the numbers from 0 to 14
 */

void more_numbers(void);

/**
 * void print_line(int n) - draws a straight line in the terminal
 */

void print_line(int n);

/**
 * void print_diagonal(int n) - draw a diagonal line on the terminal
 */

void print_diagonal(int n);

/**
 * void print_square(int size) - prints a square
 */

void print_square(int size);

/**
 * void print_triangle(int size) - prints a triangle
 */

void print_triangle(int size);

/**
 * void print_number(int n) - prints an integer
 */

void print_number(int n);

#endif
